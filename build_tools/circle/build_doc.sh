...
# Weird issues with PYTHONPATH
# https://github.com/ContinuumIO/anaconda-issues/issues/416
unset PYTHONPATH
mv ~/.local ~/old_local
...
conda create -n testenv --yes --quiet python numpy scipy \
  cython nose coverage matplotlib sphinx pillow scikit-learn sphinx
...
# hardcode removal of certain files...
# some things time out if they take too long
# this isn't amazing as far as a solution goes
# but it is a start

# This one breaks probably due to memory issues in LogReg
rm examples/plot_asirra_dataset.py
