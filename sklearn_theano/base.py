...
def test_convolution():
    convolution_filter = np.zeros([3, 3])
    convolution_filter[2, 2] = 1.

    images = np.arange(60).reshape(2, 5, 6)
...
def test_fuse():
    ...
    pipe = [PassThrough(), conv, PassThrough(), PassThrough(), relu]
    ...
    assert_array_almost_equal(results[0], convolutions[:, np.newaxis])
    assert_array_almost_equal(results[1], convolutions[:, np.newaxis])
    assert_array_almost_equal(results[2], rectified[:, np.newaxis])
